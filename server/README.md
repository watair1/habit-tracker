...
| `AI 사용 한도에 걸렸어요...` | 무료 플랜은 분당·하루 호출 제한이 있어요. 잠깐(또는 다음 날) 기다리면 풀려요. 많이 쓰려면 Google Cloud에서 결제를 켜면 한도가 크게 늘어요(flash는 매우 저렴). 한도 초과(429)일 땐 재시도해도 한도만 더 깎이므로 서버가 바로 알려주고 멈춰요. |
| `AI 서버가 잠시 붐벼요...` | 구글 쪽이 일시적으로 과부하(503)인 상태예요. 서버가 2·4·8초 간격으로 3번까지 자동 재시도하고, 그래도 안 되면 이 메시지를 보여줘요. 조금 뒤에 다시 하면 대개 됩니다. |
